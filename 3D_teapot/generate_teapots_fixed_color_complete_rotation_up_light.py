...
train_data_path = "./teapot_dataset_fixed_color_complete_rotation_up_light/train_data/"
train_data_image_path = os.path.join(train_data_path, "images")
train_data_npz_path = os.path.join(train_data_path, "data.npz")

metric_data_path = "./teapot_dataset_fixed_color_complete_rotation_up_light/metric_data/"
...
def generate_random_latents(size_):
    ...
    t2[t2 < 0] += np.pi * 2
    latents[:, 2] = t2
    #latents[:, 1] = np.random.uniform(0.0, np.pi, size_)
    #latents[:, 2] = np.random.uniform(0.0, 2 * np.pi, size_)
    return latents
